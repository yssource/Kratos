...
# Other imports
import KratosMultiphysics.CoSimulationApplication.co_simulation_solver_interfaces.co_simulation_io_factory as io_factory
...
class CoSimulationBaseSolver(object):
	...
	def __init__(self, solver_name, cosim_solver_settings):
		...
		self.SetEchoLevel( self.cosim_solver_settings["echo_level"].GetInt() )
		self.data_map = self._GetDataMap()
		self.geo_names = self._GetMeshNames()
	...
	## _GetMeshNames : Private Function to obtain the list of mesh names
	#
	#  @param self            The object pointer.
