...
namespace Kratos
{
    ...
    template <class TPrimalElement>
    void AdjointPotentialFlowElement<TPrimalElement>::CalculateSensitivityMatrix(const Variable<double>& rDesignVariable,
                                    Matrix& rOutput,
                                    const ProcessInfo& rCurrentProcessInfo)
    {
        KRATOS_TRY;
        ...
        for(unsigned int i_node = 0; i_node<NumNodes; i_node++){
            double distance_value = GetGeometry()[i_node].GetSolutionStepValue(GEOMETRY_DISTANCE);
            //perturbate distance
            pGetPrimalElement()->GetGeometry()[i_node].GetSolutionStepValue(GEOMETRY_DISTANCE) = distance_value+delta;
            //compute perturbated RHS
            pGetPrimalElement()->CalculateRightHandSide(RHS_perturbed, process_info);
            //recover distance value
            pGetPrimalElement()->GetGeometry()[i_node].GetSolutionStepValue(GEOMETRY_DISTANCE) = distance_value;
