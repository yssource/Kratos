...
#if !defined(KRATOS_COMPRESSIBLE_POTENTIAL_FLOW_ELEMENT_16_H_INCLUDED)
#define KRATOS_COMPRESSIBLE_POTENTIAL_FLOW_ELEMENT_16_H_INCLUDED
...
class CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown : public Element
{
  public:
    ...
    /// Pointer definition of CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown
    KRATOS_CLASS_POINTER_DEFINITION(CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown);
     ...
     * Constructor.
     */
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(IndexType NewId = 0){};

    /**
     * Constructor using an array of nodes
     */
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(IndexType NewId, const NodesArrayType &ThisNodes) : Element(NewId, ThisNodes){};

    /**
     * Constructor using Geometry
     */
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(IndexType NewId, GeometryType::Pointer pGeometry) : Element(NewId, pGeometry){};

    /**
     * Constructor using Properties
     */
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(IndexType NewId, GeometryType::Pointer pGeometry, PropertiesType::Pointer pProperties) : Element(NewId, pGeometry, pProperties){};

    /**
     * Copy Constructor
     */
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown const &rOther){};

    /**
     * Destructor
     */
    ~CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown() override{};
    ...
    /// Assignment operator.
    CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown &operator=(CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown const &rOther)
    {
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(NewId, GetGeometry().Create(ThisNodes), pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(NewId, pGeom, pProperties));
        KRATOS_CATCH("");
    }
    ...
    {
        KRATOS_TRY
        return Element::Pointer(new CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown(NewId, GetGeometry().Create(ThisNodes), pGetProperties()));
        KRATOS_CATCH("");
    }
    ...
    void EquationIdVector(EquationIdVectorType &rResult, ProcessInfo &CurrentProcessInfo) override
    {
        if (this->IsNot(MARKER)) //normal element
        {
            ...
            else
            {
                ...
                //Kutta elements have only positive part
                for (unsigned int i = 0; i < NumNodes; i++)
                {
                    if (distances[i] > 0.0)
                        rResult[i] = GetGeometry()[i].GetDof(POSITIVE_FACE_PRESSURE).EquationId();
    ...
    int Check(const ProcessInfo &rCurrentProcessInfo) override
    {
        ...
        if (this->Id() < 1)
        {
            KRATOS_THROW_ERROR(std::logic_error, "CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown found with Id 0 or negative", "")
        }

        if (this->GetGeometry().Area() <= 0)
        {
            std::cout << "error on CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown -> " << this->Id() << std::endl;
    ...
    std::string Info() const override
    {
        ...
        buffer << "CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown #" << Id();
        return buffer.str();
    }
    ...
    {
        rOStream << "CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown #" << Id();
    }
...
}; // Class CompressiblePotentialFlowElementWakeImplementation16NoCornerTeDown
