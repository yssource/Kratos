...
class CompressiblePotentialFlowElementWakeImplementation21NoCornerVariation : public Element
{
  public:
    ...
    void EquationIdVector(EquationIdVectorType &rResult, ProcessInfo &CurrentProcessInfo) override
    {
        ...
        else //wake element
        {
            ...
            //     for (unsigned int i = 0; i < NumNodes; i++)
            //     {
            //         rResult[2*NumNodes + i] = GetGeometry()[i].GetDof(NODAL_H).EquationId();
            //     }
            // }
            // else
            // {
    ...
    void GetDofList(DofsVectorType &rElementalDofList, ProcessInfo &CurrentProcessInfo) override
    {
        ...
        else //wake element
        {
            ...
            //     for (unsigned int i = 0; i < NumNodes; i++)
            //     {
            //         rElementalDofList[2*NumNodes + i] = GetGeometry()[i].pGetDof(NODAL_H);
            //     }
        ...
        else //it is a wake element
        {
            ...
            if (this->Is(STRUCTURE))
            {
                ...
                for (unsigned int i = 0; i < NumNodes; ++i)
                {
                    ...
                    //     for (unsigned int j = 0; j < NumNodes; ++j)
                    //     {
                    //         rLeftHandSideMatrix(i + 2 * NumNodes, j + 2 * NumNodes) = lhs_total(i, j);
                    //     }
                    //     // if(GetGeometry()[i].FastGetSolutionStepValue(TRAILING_EDGE))
                    //     // {
                    //     //     for (unsigned int j = 0; j < NumNodes; ++j)
                    ...
                    //     //         {
                    //     //             rLeftHandSideMatrix(i + 2 * NumNodes, j + 2 * NumNodes) = lhs_total(i, j);
                    //     //             rLeftHandSideMatrix(i + 2 * NumNodes, j + NumNodes) = -lhs_total(i, j);
                    //     //         }
                    //     //     }
  ...
  protected:
    ...
    void GetValuesOnTripleSplitElement(Vector &split_element_values, const array_1d<double, NumNodes> &distances)
    {
        for (unsigned int i = 0; i < NumNodes; i++)
        {
            if (distances[i] > 0)
        ...
        for (unsigned int i = 0; i < NumNodes; i++)
        {
            split_element_values[2*NumNodes + i] = GetGeometry()[i].FastGetSolutionStepValue(NODAL_H);
        }
    }
