...
class DefineWakeProcess(KratosMultiphysics.Process):
    ...
    def Execute(self):
        ...
        for elem in self.all_trailing_edge_model_part.Elements:
            counter_zero_velocity_condition_nodes = 0
            for elnode in elem.GetNodes():
                if(elnode.GetSolutionStepValue(KratosMultiphysics.CompressiblePotentialFlowApplication.ZERO_VELOCITY_CONDITION) == True):
                    counter_zero_velocity_condition_nodes += 1
            if(counter_zero_velocity_condition_nodes > 1):
                elem.SetValue(KratosMultiphysics.CompressiblePotentialFlowApplication.ZERO_VELOCITY_CONDITION, True)
