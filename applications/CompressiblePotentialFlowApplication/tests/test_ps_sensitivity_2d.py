...
class TestCase(KratosUnittest.TestCase):
    ...
    def _get_lift(self,direction,lift_file_name):
        with open(lift_file_name, 'r') as lift_file:
            lines = lift_file.readlines()
        lines = lines[1:] # remove header
        
        for line in lines:
            components = line.split()
            lift = float(components[0])
            
        #drag = direction[0] * dx + direction[1] * dy + direction[2] * dz
        return lift

    def _compute_finite_difference_lift_sensitivity(self,node_ids,step_size,model_part_file_name,lift_direction,lift_file_name):
        sensitivity = []
        # unperturbed lift
        self.solve(model_part_file_name)
        sensitivity = 0
        #lift0 = self._get_lift(lift_direction,lift_file_name)
        ...
        #    self.solve(model_part_file_name)
        #    drag = self._get_time_averaged_drag(lift_direction,lift_file_name)
        #    node_sensitivity.append((drag - lift0) / step_size)
        #    # Y + h
    ...
    def test_OneElement(self):
        with ControlledExecutionScope(os.path.dirname(os.path.realpath(__file__))):
            ...
            FDSensitivity = self._compute_finite_difference_lift_sensitivity([1],step_size,'./test_ps_sensitivity_2d/one_element_test',[0.0,1.0,0.0],'./test_ps_sensitivity_2d/one_element_test.dat')
            #self.assertAlmostEqual(Sensitivity[0][0], FDSensitivity[0][0], 4)
            #self.assertAlmostEqual(Sensitivity[0][1], FDSensitivity[0][1], 4)
            self._remove_file("./test_ps_sensitivity_2d/one_element_test_0.h5")
            self._remove_file("./test_ps_sensitivity_2d/one_element_test.dat")
            self._remove_file("./test_ps_sensitivity_2d/one_element_test.time")
    ...
    #        # calculate sensitivity by finite difference
            step_size = 0.00000001
            FDSensitivity = self._compute_finite_difference_lift_sensitivity([223],step_size,'./test_ps_sensitivity_2d/naca0012_test',[0.0,1.0,0.0],'./test_ps_sensitivity_2d/naca0012_test.dat')
    #        self.assertAlmostEqual(Sensitivity[0][0], FDSensitivity[0][0], 4)
    #        self.assertAlmostEqual(Sensitivity[0][1], FDSensitivity[0][1], 2)
            self._remove_file("./test_ps_sensitivity_2d/naca0012_test_0.h5")
            self._remove_file("./test_ps_sensitivity_2d/naca0012_test.dat")
            self._remove_file("./test_ps_sensitivity_2d/naca0012_test.time")
    
    def test_SteadyNacaFine(self):
        with ControlledExecutionScope(os.path.dirname(os.path.realpath(__file__))):
    #        # solve fluid
    #        self.solve('test_vms_sensitivity_2d/steady_cylinder_test')
    #        # solve adjoint
    ...
    #        # calculate sensitivity by finite difference
            step_size = 0.00000001
            FDSensitivity = self._compute_finite_difference_lift_sensitivity([223],step_size,'./test_ps_sensitivity_2d/naca0012fine_test',[0.0,1.0,0.0],'./test_ps_sensitivity_2d/naca0012fine_test.dat')
    #        self.assertAlmostEqual(Sensitivity[0][0], FDSensitivity[0][0], 4)
    #        self.assertAlmostEqual(Sensitivity[0][1], FDSensitivity[0][1], 2)
            self._remove_file("./test_ps_sensitivity_2d/naca0012fine_test_0.h5")
    #        self._remove_file("./test_ps_sensitivity_2d/naca0012fine_test_0.dat")
            self._remove_file("./test_ps_sensitivity_2d/naca0012fine_test.time")
