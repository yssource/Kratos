...
namespace Kratos
{
   ...
   // ***************************************************************************
   // Set Value (Vector)
   void BorjaModel::SetValue( const Variable<Vector> & rThisVariable, const Vector & rValue, const ProcessInfo& rCurrentProcessInfo)
   {
      KRATOS_TRY
   ...
   void BorjaModel::SetStressState( MatrixType & rHenckyStrain, const double & rReferencePressure, const double & rSwellingSlope, const double & rAlphaShear, const double & rConstantShearModulus)
   {
      ...
      Vector Objective(3);
      Objective(0) = mInitialStressState(0) + mInitialStressState(1) + mInitialStressState(2);
      Objective(0) /= 3.0;
      Objective(1) = mInitialStressState(1) - Objective(0);
      Objective(2) = mInitialStressState(2) - Objective(0);
