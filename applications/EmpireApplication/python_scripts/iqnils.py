...
class IQNILS:
    ...
    def __init__( self, iteration_horizon = 15, timestep_horizon = 1, alpha = 0.3 ):
            if "iteration_horizon" in convergence_accelerator_settings:
                iteration_horizon = convergence_accelerator_settings["iteration_horizon"]
            else:
                iteration_horizon = 15
            if "timestep_horizon" in convergence_accelerator_settings:
                timestep_horizon = convergence_accelerator_settings["timestep_horizon"]
            else:
                timestep_horizon = 1
...
def Create(convergence_accelerator_settings):
    return IQNILS(convergence_accelerator_settings)
