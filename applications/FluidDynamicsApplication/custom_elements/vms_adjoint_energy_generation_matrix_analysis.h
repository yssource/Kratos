                              ...
                              const Vector& rVector2 )
{
    ...
    // energy /= (sqrt(inner_prod(rVector1, rVector1)) * sqrt(inner_prod(rVector2, rVector2)));
        ...
        for (unsigned int i=0; i<TDim; ++i)
        {
            for (unsigned int c=0; c < TNumNodes; ++c)
            {
                for (unsigned int k=0; k < TDim; ++k)
                {
                    ...
                    rMatrix(a*TDim+i, c*TDim+k) = val*weight;
                }
            }
        }
    }
        ...
        for (unsigned int i=0; i<TDim; ++i)
        {
            for (unsigned int c=0; c < TNumNodes; ++c){
                ...
                for (unsigned int k=0; k < TDim; ++k)
                {
                    ...
                    if (i==k)
                        val += diag;
                    rMatrix(a*TDim+i, c*TDim+k) = -viscosity*val*weight;
                }
            }
        }
    }
                    ...
                    const ProcessInfo& rCurrentProcessInfo )
{
    ...
    if (MatrixId == 100)
    {
        ...
        var_matrix_energy = &STABILIZATION_ANALYSIS_MATRIX_PRIMAL_GRADIENT_OF_VMS_STEADY_TERM_ENERGY;
        var_eigen_min = &STABILIZATION_ANALYSIS_MATRIX_PRIMAL_GRADIENT_OF_VMS_STEADY_TERM_EIGEN_MIN;
        var_eigen_max = &STABILIZATION_ANALYSIS_MATRIX_PRIMAL_GRADIENT_OF_VMS_STEADY_TERM_EIGEN_MAX;
    }

    if (MatrixId == 101)
    {
        current_matrix = this->GetValue(STABILIZATION_ANALYSIS_MATRIX_LHS_BEFORE_STABILIZATION);
        matrix_energy = CalculateMatrixEnergy(adjoint_vector, current_matrix, adjoint_vector);
        var_matrix_energy = &STABILIZATION_ANALYSIS_MATRIX_LHS_BEFORE_STABILIZATION_ENERGY;
        var_eigen_min = &STABILIZATION_ANALYSIS_MATRIX_LHS_BEFORE_STABILIZATION_EIGEN_MIN;
        var_eigen_max = &STABILIZATION_ANALYSIS_MATRIX_LHS_BEFORE_STABILIZATION_EIGEN_MAX;
    }

    if (MatrixId == 102)
    {
        current_matrix = this->GetValue(STABILIZATION_ANALYSIS_MATRIX_LHS_AFTER_STABILIZATION);
        matrix_energy = CalculateMatrixEnergy(adjoint_vector, current_matrix, adjoint_vector);
        var_matrix_energy = &STABILIZATION_ANALYSIS_MATRIX_LHS_AFTER_STABILIZATION_ENERGY;
        var_eigen_min = &STABILIZATION_ANALYSIS_MATRIX_LHS_AFTER_STABILIZATION_EIGEN_MIN;
        var_eigen_max = &STABILIZATION_ANALYSIS_MATRIX_LHS_AFTER_STABILIZATION_EIGEN_MAX;
    }

    if (MatrixId == 203)
    {
        current_matrix = this->GetValue(STABILIZATION_ANALYSIS_MATRIX_CUSTOM);
        matrix_energy = CalculateMatrixEnergy(adjoint_vector, current_matrix, adjoint_vector);
        var_matrix_energy = &STABILIZATION_ANALYSIS_MATRIX_CUSTOM_ENERGY;
        var_eigen_min = &STABILIZATION_ANALYSIS_MATRIX_CUSTOM_EIGEN_MIN;
        var_eigen_max = &STABILIZATION_ANALYSIS_MATRIX_CUSTOM_EIGEN_MAX;
    }

    if (MatrixId == 204)
    {
        current_matrix = this->GetValue(STABILIZATION_ANALYSIS_MATRIX_DIFFUSION);
        matrix_energy = CalculateMatrixEnergy(adjoint_vector, current_matrix, adjoint_vector);
        var_matrix_energy = &STABILIZATION_ANALYSIS_MATRIX_DIFFUSION_ENERGY;
        var_eigen_min = &STABILIZATION_ANALYSIS_MATRIX_DIFFUSION_EIGEN_MIN;
        var_eigen_max = &STABILIZATION_ANALYSIS_MATRIX_DIFFUSION_EIGEN_MAX;
    }
    ...
    this->SetValue(STABILIZATION_ANALYSIS_MATRIX_ADJOINT_VECTOR, adjoint_vector);
}

void ProcessSymmetricMatrices(const ProcessInfo& rCurrentProcessInfo)
{
    for (unsigned int iMatrix = 1; iMatrix < 22; ++iMatrix)
    {
        ComputeMatrix(iMatrix, rCurrentProcessInfo);
    }

    ComputeMatrix(100, rCurrentProcessInfo);
    ComputeMatrix(101, rCurrentProcessInfo);
    ComputeMatrix(102, rCurrentProcessInfo);
    ComputeMatrix(203, rCurrentProcessInfo);
    ComputeMatrix(204, rCurrentProcessInfo);
}
