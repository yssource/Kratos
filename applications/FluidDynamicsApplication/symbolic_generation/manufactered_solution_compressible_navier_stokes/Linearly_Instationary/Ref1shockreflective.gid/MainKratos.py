...
def SetInitialConditions(time):
    ...
    for node in main_model_part.GetSubModelPart("Parts_Parts_Auto1").Nodes:
        ...
        node.SetSolutionStepValue(MOMENTUM_X,32*t + 32*x + 64*y + 160 )
        node.SetSolutionStepValue(MOMENTUM_Y,-8*t + 16*x - 32*y + 24)
        node.SetSolutionStepValue(TOTAL_ENERGY,t + 100*x - 500*y + 36500)
        
        
def SetBoundaryConditions(time):
    t = time
    print("tempo", t)
...
# Setting n-2 time step conditions
SetInitialConditions(time)
SetBoundaryConditions(time)

gid_output.ExecuteInitializeSolutionStep()      # Output of step 0 in GiD 
gid_output.ExecuteFinalizeSolutionStep()        # Befor solving, Time step n-2
...
while(time <= end_time):
    ...
    if (time<delta_time*2):
        SetInitialConditions(time)              # Set Initial conditions only for n-1
        
    SetBoundaryConditions(time)                 # Update of the BCs at every time step
