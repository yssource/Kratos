...
//         array_1d<double, TNumNodes > tau1,tau2,tau3; // stabilization matrix parameters
                              ...
                              ProcessInfo& rCurrentProcessInfo) override
    {
        ...
        for(unsigned int igauss = 0; igauss<ngauss; igauss++)
        {
            ...
            row(GetGeometry().ShapeFunctionsValues(GeometryData::GI_GAUSS_4), igauss);
            double xgauss = 0;
            double ygauss = 0;
            ...
            double x = xgauss;
            double y = ygauss;
            data.f_gauss[0] =  1.0*(28.0*pow(x,4) + 224.0*pow(x,3)*y + 179.199999999999*pow(x,3) + \
            672.000000000001*x*x*y*y + 1075.2*x*x*y + 430.079999999999*x*x + 896.000000000001*x*pow(y,3) +\
             2150.4*x*y*y + 1720.31999999999*x*y + 458.752*x + 448.0*pow(y,4) + 1433.6*pow(y,3) + 1720.31999999999*y*y + 917.504000000001*y + 183.5008)/(pow((0.5*x + 1.0*y + 0.8),3)*pow((1.0*x + 2.0*y + 1.6),2));
            
            data.f_gauss[1] =  -1.0*(94.0*pow(x,4) + 752.0*pow(x,3)*y + 601.6*pow(x,3) + 2256.0*x*x*y*y + 3609.6*x*x*y + 1443.84*x*x + 3008.0*x*pow(y,3) + 7219.2*x*y*y +\
             5775.36000000001*x*y + 1540.096*x + 1504.0*pow(y,4) + 4812.8*pow(y,3) + 5775.36*y*y + 3080.192*y + 616.0384)/(pow((0.5*x + 1.0*y + 0.8),3)*pow((1.0*x + 2.0*y + 1.6),2));

            data.r_gauss = 1.0*(18000.0*pow(x,3) + 108000.0*x*x*y + 86400.0*x*x + 216000.0*x*y*y + 345600.0*x*y + 138240.0*x + 144000.0*pow(y,3) + 345600.0*y*y + 276480.0*y + 73728.0)/(0.25*pow(x,4) + 2.0*pow(x,3)*y + 1.6*pow(x,3) + 6.0*x*x*y*y + 9.6*x*x*y + 3.84*x*x + 8.0*x*pow(y,3) + 19.2*x*y*y\
             + 15.36*x*y + 4.096*x + 4.0*pow(y,4) + 12.8*pow(y,3) + 15.36*y*y + 8.192*y + 1.6384);
        ...
        */
        ...
        for(unsigned int igauss = 0; igauss<Ncontainer.size2(); igauss++)
        {
            ...
            ComputeGaussPointRHSContribution(rhs_local, data);
            ComputeGaussPointLHSContribution(lhs_local, data);
            //std::cout<<this->Id()<<"LOCAL "<<rhs_local<<"\n"<<std::endl;
            // here we assume that all the weights of the gauss points are the same so we multiply at the end by Volume/n_nodes
            noalias(rLeftHandSideMatrix) += lhs_local;
            noalias(rRightHandSideVector) += rhs_local;
        }
