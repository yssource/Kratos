...
namespace Kratos {
	namespace Testing {
		...
		{
            std::cout<<"\n\nTest For Constant Variables"<<std::endl;
...
//             std::cout<<*pElement<<std::endl; //Info on element
            ...
            modelPart.CloneTimeStep(2*delta_time); //to simulate the zero acceleration considers constant variables in steps
...
//             std::cout<<*pElement<<std::endl; //Info on element
			...
			for (unsigned int i=0; i<RHS.size(); ++i)
				sum_RHS += RHS[i]*RHS[i];
            sum_RHS = sqrt(sum_RHS);
			KRATOS_CHECK_NEAR(sum_RHS, 0.0, 1e-11);
	    ...
	    /** Checks the CompressibleNavierStokes2D3N element.
		 * Checks the LHS and RHS stationary solid rigid rotations.
         * Test for Air at 20 degrees and Mach 0.5
		 */	  
        /*
        KRATOS_TEST_CASE_IN_SUITE(ElementCompressibleNavierStokes2D3NStationaryRotation, FluidDynamicsApplicationFastSuite)
		{
            std::cout<<"\n\nTest For Stationary Rigid Movements"<<std::endl;
			ModelPart modelPart("Main");
			modelPart.SetBufferSize(3);
...
//             for(unsigned int i =0; i<elemNodes.size(); ++i){
                      
//             std::cout<<pElement->GetGeometry()[1].Coordinate(1)<<std::endl;
//             std::cout<<pElement->GetGeometry()[1].Coordinate(2)<<std::endl;
//             std::cout<<pElement->GetGeometry()[1].Coordinate(3)<<std::endl;
//             modelPart.GetNode(1);        
...
//             density*(1.005-0.718)*T/(1.4-1)+(momentum[0]*momentum[0]+momentum[1]*momentum[1]+momentum[2]*momentum[2])/(2*density);   	
            momentum_n[0] = 1.0; momentum_n[1] = 1.0; momentum_n[2] = 1.0; //               TO BE CHECK Casual value for the previous steps 
...
//             std::cout<<"\nMach = "<<Ma<<std::endl;
			...
			for (NodeIteratorType it_node=modelPart.NodesBegin(); it_node<modelPart.NodesEnd(); ++it_node){
                ...
                it_node->FastGetSolutionStepValue(MOMENT,1) = momentum_n;
				it_node->FastGetSolutionStepValue(MOMENT,2) = momentum_n;
                total_energy = density*(1.005-0.718)*T/(1.4-1)+(momentum[0]*momentum[0]+momentum[1]*momentum[1]+momentum[2]*momentum[2])/(2*density);
				it_node->FastGetSolutionStepValue(TOTAL_ENERGY) = total_energy;
				it_node->FastGetSolutionStepValue(TOTAL_ENERGY,1) = 1;
				it_node->FastGetSolutionStepValue(TOTAL_ENERGY,2) = 1;
				it_node->FastGetSolutionStepValue(BODY_FORCE) = f_ext;
