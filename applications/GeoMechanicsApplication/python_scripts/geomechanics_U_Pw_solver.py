...
class UPwSolver(object):
    ...
    def AddVariables(self):
        ...
        # Add dynamic variables
        self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.DT_WATER_PRESSURE)
        # Add variables for the water conditions
        self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NORMAL_FLUID_FLUX)
        ## Other variables
        ...
        if(self.settings["nodal_smoothing"].GetBool() == True):
            ...
            self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NODAL_CAUCHY_STRESS_TENSOR)
            self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NODAL_DAMAGE_VARIABLE)
            self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NODAL_JOINT_AREA)
            self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NODAL_JOINT_WIDTH)
            self.main_model_part.AddNodalSolutionStepVariable(KratosGeo.NODAL_JOINT_DAMAGE)
    ...
    def ImportModelPart(self):
        ...
        if(self.settings["model_import_settings"]["input_type"].GetString() == "mdpa"):
            ...
            model = KratosMultiphysics.Model() #TODO: this will break 
            ...
            read_materials_process.Factory(read_settings, model )
    ...
    def Initialize(self):
        ...
        if(self.settings["nodal_smoothing"].GetBool() == True):
            self.main_model_part.ProcessInfo.SetValue(KratosGeo.NODAL_SMOOTHING, True)
        else:
            self.main_model_part.ProcessInfo.SetValue(KratosGeo.NODAL_SMOOTHING, False)
    ...
    def _ExecuteAfterReading(self):
        ...
        check_and_prepare_model_process_geo.CheckAndPrepareModelProcess(self.main_model_part, aux_params).Execute()
        ...
        if(scheme_type == "Newmark"):
            ...
            if(solution_type == "Quasi-Static"):
                if(rayleigh_m<1.0e-20 and rayleigh_k<1.0e-20):
                    ...
                    scheme = KratosGeo.NewmarkQuasistaticDampedUPwScheme(beta,gamma,theta,rayleigh_m,rayleigh_k)
            else:
                scheme = KratosGeo.NewmarkDynamicUPwScheme(beta,gamma,theta,rayleigh_m,rayleigh_k)
        else:
    ...
    def _ConstructSolver(self, builder_and_solver, scheme, convergence_criterion, strategy_type):
        ...
        if strategy_type == "Newton-Raphson":
            ...
            if nonlocal_damage:
                ...
                solver = KratosGeo.GeoMechanicsNewtonRaphsonNonlocalStrategy(self.main_model_part,
                                                                               scheme,
                                                                               self.linear_solver,
                                                                               convergence_criterion,
                                                                               builder_and_solver,
                ...
                solver = KratosGeo.GeoMechanicsNewtonRaphsonStrategy(self.main_model_part,
                                                                       scheme,
                                                                       self.linear_solver,
                                                                       convergence_criterion,
                                                                       builder_and_solver,
            ...
            if nonlocal_damage:
                ...
                self.strategy_params.AddValue("characteristic_length",self.settings["characteristic_length"])
                self.strategy_params.AddValue("search_neighbours_step",self.settings["search_neighbours_step"])
                solver = KratosGeo.GeoMechanicsRammArcLengthNonlocalStrategy(self.main_model_part,
                                                                             scheme,
                                                                             self.linear_solver,
                                                                             convergence_criterion,
                                                                             builder_and_solver,
                ...
                solver = KratosGeo.GeoMechanicsRammArcLengthStrategy(self.main_model_part,
                                                                     scheme,
                                                                     self.linear_solver,
                                                                     convergence_criterion,
                                                                     builder_and_solver,
