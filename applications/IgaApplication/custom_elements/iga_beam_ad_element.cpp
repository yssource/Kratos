    ...
    const bool ComputeRightHandSide)
{
    ...
    const double Phi_1 = properties[PHI_0_DER];

    const Vector3d A01 = MapVector(properties[T0]);
    const Vector3d A01_1 = MapVector(properties[T0_DER]);
    const Vector3d A02 = MapVector(properties[N0]);
