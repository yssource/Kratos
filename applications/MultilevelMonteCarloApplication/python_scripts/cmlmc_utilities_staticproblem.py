...
def AddResultsAux_Task(QoI_finer,QoI_coarser):
    difference_QoI_value = QoI_finer - QoI_coarser
    return difference_QoI_value
    ...
    def AddResults(self,simulation_results,level):
        ...
        for lev in range (level+1):
            if (level == 0):
                ...
                difference_QoI_value = AddResultsAux_Task(simulation_results.QoI[level],simulation_results.QoI[level - 1])
                # difference_QoI_value = simulation_results.QoI[level] - simulation_results.QoI[level - 1]
            time_ML_value = simulation_results.time_ML[level]
