...
import cmlmc_utilities_staticproblem as mlmc

# Import refinement library
import adaptive_refinement_utilities_staticproblem as refinement
...
def ExecuteMultilevelMonteCarloAnalisysOneLevelWithRefinement_Task(serialized_coarse_model,serialized_coarse_parameters,sample,current_MLMC_simulation_time,current_level,finest_level,size_meshes):
    start_time = time.time()
    '''initialize single Monte Carlo simualtion time'''
    start_MC_simulation_time = time.time()
    '''overwrite the old model serializer with the unpickled one'''
    # model_serializer = pickle.loads(pickled_coarse_model)
    current_model = KratosMultiphysics.Model()
    serialized_coarse_model.Load("ModelSerialization",current_model)
    del(serialized_coarse_model)
    ...
    serialized_coarse_parameters.Load("ParametersSerialization",current_parameters)
    del(serialized_coarse_parameters)
    end_time = time.time()    
    print("curr lev",current_level,"finest lev",finest_level,"load time",end_time - start_time)
    '''run the simulation'''
    start_time = time.time()
    ...
    end_time = time.time()
    print("curr lev",current_level,"finest lev",finest_level,"run time",end_time - start_time)
    ...
    if (current_level < finest_level):
        ...
        model_refined = refinement.compute_refinement_hessian_metric(simulation,size_meshes[current_level+1],size_meshes[current_level])
    ...
    del(simulation.model,simulation)
    end_time = time.time()
    print("curr lev",current_level,"finest lev",finest_level,"save time",end_time - start_time)
    '''evaluate time of the single simulation and refinement (if occurred)'''
    end_MC_simulation_time = time.time()
    current_MC_simulation_time = end_MC_simulation_time - start_MC_simulation_time
    # print(current_MLMC_simulation_time,current_MC_simulation_time)
    current_MLMC_simulation_time = current_MLMC_simulation_time + current_MC_simulation_time
    # print(current_MLMC_simulation_time)
    return QoI,current_MLMC_simulation_time,serialized_model,serialized_parameters
...
'''
def ExecuteMultilevelMonteCarloAnalisys(finest_level,pickled_coarse_model,pickled_coarse_parameters,size_meshes):
    serialized_coarse_model = pickle.loads(pickled_coarse_model)
    serialized_coarse_parameters = pickle.loads(pickled_coarse_parameters)
    '''generate the sample'''
    ...
    MLMC_time = []
    '''initialize time'''
    current_MLMC_time = 0.0
    if(finest_level == 0):
        '''run the problem and refine if necessary'''
        lev = 0
        QoI_to_append,time_MLMC_to_append,serialized_finer_model,serialized_finer_parameters =\
        ExecuteMultilevelMonteCarloAnalisysOneLevelWithRefinement_Task(serialized_coarse_model,serialized_coarse_parameters,sample,current_MLMC_time,lev,finest_level,size_meshes)
        '''append the results to their list'''
        QoI.append(QoI_to_append)
        MLMC_time.append(time_MLMC_to_append)
    else:
        print("="*40)
        serialized_coarse_model_loop = serialized_coarse_model
        serialized_coarse_parameters_loop = serialized_coarse_parameters
        for lev in range(finest_level+1):
            '''run the problem and refine if necessary'''
            print("Refinement level {}".format(finest_level+1))
            print(len(pickle.dumps(serialized_coarse_model_loop, 2)))
            print("**"*4)
            QoI_to_append,time_MLMC_to_append,serialized_finer_model,serialized_finer_parameters =\
            ExecuteMultilevelMonteCarloAnalisysOneLevelWithRefinement_Task(serialized_coarse_model_loop,serialized_coarse_parameters_loop,sample,current_MLMC_time,lev,finest_level,size_meshes)
            print(len(pickle.dumps(serialized_finer_model, 2)))
            '''append the results to their list'''
            QoI.append(QoI_to_append)
            MLMC_time.append(time_MLMC_to_append)
            '''update time'''
            current_MLMC_time = time_MLMC_to_append
            '''update serialized model and parameters'''
            serialized_coarse_model_loop = serialized_finer_model
            serialized_coarse_parameters_loop = serialized_finer_parameters
    del(serialized_coarse_model,serialized_coarse_parameters,serialized_finer_model,serialized_finer_parameters)
    '''prepare results of the simulation in the MultilevelMonteCarloResults class'''
    mlmc_results_class.finer_level = finest_level
    for lev in range(finest_level+1):
        mlmc_results_class.time_ML.append(MLMC_time[lev])
...
if __name__ == '__main__':
    ...
    #parameter_file_name = "/home/ramela/Documents/Kratos_3/applications/MultilevelMonteCarloApplication/tests/MeshCoarse8Nodes/ProjectParameters.json"    '''create a serialization of the model and of the project parameters'''
