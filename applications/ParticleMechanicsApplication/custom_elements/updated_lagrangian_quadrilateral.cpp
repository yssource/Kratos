...
void UpdatedLagrangianQuadrilateral::CalculateExplicitKinematics(GeneralVariables & rVariables, ProcessInfo & rCurrentProcessInfo)
{
	KRATOS_TRY
	...
	bool isCompressible = false; //add some test to see if we are considering compressible or not
	if (isCompressible)
	{
		...
		rVariables.detF = MathUtils<double>::DetMat(rVariables.F);
		

		//PJW - change to incremental detF stored in rVariables!
		// Adjust volume and density to conserve mass
		double& MP_Volume = this->GetValue(MP_VOLUME); //PJW, needed for explicit force calculation
		MP_Volume *= rVariables.detF;
		this->SetValue(MP_VOLUME, MP_Volume);
		...
		for (unsigned int i = 0; i < number_of_nodes; i++)
		{
			...
			nodal_force_internal_normal[0] = MP_Volume * 
				(MP_Stress[0] * Variables.DN_DX(i, 0) + 
					MP_Stress[2] * Variables.DN_DX(i, 1));
...
void UpdatedLagrangianQuadrilateral::UpdateGaussPointExplicit(GeneralVariables & rVariables, const ProcessInfo & rCurrentProcessInfo)
{
	KRATOS_TRY
	...
	for (unsigned int i = 0; i < number_of_nodes; i++)
	{
		if (mN[i] > std::numeric_limits<double>::epsilon())
		{
			...
			if (nodal_mass > std::numeric_limits<double>::epsilon())
			{
				...
				for (unsigned int j = 0; j < dimension; j++)
				{
					MP_Acceleration[j] += mN[i] * r_current_residual[j] / nodal_mass;
					delta_xg[j] += delta_time * mN[i] * r_nodal_momenta[j] / nodal_mass;
				}
			}
		}
	}
	...
	for (unsigned int j = 0; j < dimension; j++)
	{
		MP_Velocity[j] = MP_PreviousVelocity[j] + delta_time * MP_Acceleration[j];
...
void UpdatedLagrangianQuadrilateral::AddExplicitContribution(
...
)
{
    ...
    if (rDestinationVariable == NODAL_MASS) {
        MatrixType element_mass_matrix(local_size,local_size);
...
)
{
    ...
    if (rRHSVariable == RESIDUAL_VECTOR && rDestinationVariable == FORCE_RESIDUAL) {
        ...
        for (int i = 0; i < number_of_nodes; ++i) {
            double& r_nodal_mass = rGeom[i].GetValue(NODAL_MASS);
            array_1d<double, 3>& r_nodal_inertia = rGeom[i].GetValue(NODAL_INERTIA);
            int index = i * dimension;
