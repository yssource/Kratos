...
// Include Base h
#include "evm_log_epsilon_element.h"
...
EvmLogEpsilonElement<TDim, TNumNodes>::EvmLogEpsilonElement(IndexType NewId)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmLogEpsilonElement<TDim, TNumNodes>::EvmLogEpsilonElement(EvmLogEpsilonElement<TDim, TNumNodes> const& rOther)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmLogEpsilonElement<TDim, TNumNodes>::~EvmLogEpsilonElement()
{
}
...
/// Assignment operator.
template <unsigned int TDim, unsigned int TNumNodes>
EvmLogEpsilonElement<TDim, TNumNodes>& EvmLogEpsilonElement<TDim, TNumNodes>::operator=(
    EvmLogEpsilonElement<TDim, TNumNodes> const& rOther)
{
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmLogEpsilonElement<TDim, TNumNodes>::Create(
    IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    KRATOS_TRY
    return Kratos::make_shared<EvmLogEpsilonElement>(
...
{
    KRATOS_TRY
    return Kratos::make_shared<EvmLogEpsilonElement>(NewId, pGeom, pProperties);
    KRATOS_CATCH("");
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmLogEpsilonElement<TDim, TNumNodes>::Clone(IndexType NewId,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::EquationIdVector(EquationIdVectorType& rResult,
                                                             ProcessInfo& CurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rResult[i] = Element::GetGeometry()[i]
                         .GetDof(TURBULENT_LOG_ENERGY_DISSIPATION_RATE)
                         .EquationId();
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::GetDofList(DofsVectorType& rElementalDofList,
                                                       ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rElementalDofList[i] =
            Element::GetGeometry()[i].pGetDof(TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::GetValuesVector(VectorType& rValues, int Step)
{
    this->GetFirstDerivativesVector(rValues, Step);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::GetFirstDerivativesVector(VectorType& rValues, int Step)
{
    ...
    for (IndexType iNode = 0; iNode < TNumNodes; ++iNode)
    {
        rValues[LocalIndex++] = rGeom[iNode].FastGetSolutionStepValue(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE, Step);
    }
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::GetSecondDerivativesVector(VectorType& rValues, int Step)
{
    if (rValues.size() != TNumNodes)
        rValues.resize(TNumNodes, false);
    ...
    {
        rValues[LocalIndex++] = rGeom[iNode].FastGetSolutionStepValue(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2, Step);
    }
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix,
                                                                  ProcessInfo& rCurrentProcessInfo)
{
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateRightHandSide(VectorType& rRightHandSideVector,
                                                                   ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        const array_1d<double, 3> effective_convective_velocity = this->GetConvectiveVelocity(
            epsilon_sigma, gauss_shape_functions, r_shape_derivatives);
        ...
        const double log_tke =
            this->EvaluateInPoint(TURBULENT_LOG_KINETIC_ENERGY, gauss_shape_functions);
        ...
        const double f2 =
            EvmLogKepsilonModelUtilities::CalculateF2(log_tke, nu, log_epsilon);
        const double gamma =
            EvmLogKepsilonModelUtilities::CalculateGamma(log_tke, log_epsilon);
        const double effective_kinematic_viscosity = nu + nu_t / epsilon_sigma;

        const double source = this->CalculateSourceTerm(
            c1, c2, f2, gamma, nu_t, nu, y_plus, wall_distance, log_epsilon, r_shape_derivatives);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesContributions(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesLHS(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesRHS(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesContributions(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesLHS(
    MatrixType& rLeftHandSideMatrix, ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesRHS(
    VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    if (rRightHandSideVector.size() != 0)
        rRightHandSideVector.resize(0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateMassMatrix(MatrixType& rMassMatrix,
                                                                ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateDampingMatrix(MatrixType& rDampingMatrix,
                                                                   ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        array_1d<double, 3> log_epsilon_gradient;
        this->CalculateGradient(log_epsilon_gradient, TURBULENT_LOG_ENERGY_DISSIPATION_RATE,
                                r_shape_derivatives);
        const double log_epsilon_gradient_norm = norm_2(log_epsilon_gradient);

        if (log_epsilon_gradient_norm > std::numeric_limits<double>::epsilon() &&
            effective_velocity_magnitude_square > std::numeric_limits<double>::epsilon())
        {
            const double relaxed_log_epsilon_acceleration =
                this->EvaluateInPoint(RANS_AUXILIARY_VARIABLE_2, gauss_shape_functions);

            const double log_tke = this->EvaluateInPoint(
            ...
            const double gamma =
                EvmLogKepsilonModelUtilities::CalculateGamma(log_tke, log_epsilon);
            const double source = this->CalculateSourceTerm(
                c1, c2, f2, gamma, nu_t, nu, y_plus, wall_distance, log_epsilon,
                r_shape_derivatives);
            Vector nodal_log_epsilon;
            this->GetValuesVector(nodal_log_epsilon);

            double residual = relaxed_log_epsilon_acceleration;
            residual += inner_prod(velocity_convective_terms, nodal_log_epsilon);
            residual -= source;
            residual = std::abs(residual);
            residual /= log_epsilon_gradient_norm;
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::CalculateLocalVelocityContribution(
    MatrixType& rDampingMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int iNode = 0; iNode < TNumNodes; ++iNode)
        U[iNode] = this->GetGeometry()[iNode].FastGetSolutionStepValue(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
int EvmLogEpsilonElement<TDim, TNumNodes>::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY

    KRATOS_ERROR_IF(this->Id() < 1)
        << "EvmLogEpsilonElement found with Id 0 or negative" << std::endl;

    KRATOS_ERROR_IF(this->Element::GetGeometry().Area() <= 0)
        << "On EvmLogEpsilonElement -> " << this->Id()
...
template <unsigned int TDim, unsigned int TNumNodes>
std::string EvmLogEpsilonElement<TDim, TNumNodes>::Info() const
{
    std::stringstream buffer;
    buffer << "EvmLogEpsilonElement #" << Element::Id();
    return buffer.str();
}
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "EvmLogEpsilonElement #" << Element::Id();
}

/// Print object's data.

template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::PrintData(std::ostream& rOStream) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
array_1d<double, 3> EvmLogEpsilonElement<TDim, TNumNodes>::GetConvectiveVelocity(
    ...
    const Matrix& rGaussShapeDerivatives) const
{
    ...
    return velocity - effective_kinematic_viscosity * log_epsilon_gradient;
}

template <unsigned int TDim, unsigned int TNumNodes>
double EvmLogEpsilonElement<TDim, TNumNodes>::CalculateSourceTerm(
    const double c1,
    const double c2,
    const double f2,
    const double gamma,
    ...
    const Matrix& rShapeDerivatives) const
{
    ...
    const double P = EvmLogKepsilonModelUtilities::CalculateSourceTerm<TDim>(velocity_gradient_matrix, turbulent_kinematic_viscosity);

    const double reaction = c2 * f2 * gamma + 2.0 * kinematic_viscosity *
                                                  std::exp(-0.5 * y_plus) /
                                                  std::pow(wall_distance, 2);
    const double source =
        c1 * gamma * std::exp(-1.0 * log_turbulent_energy_dissipation_rate) * P;
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::save(Serializer& rSerializer) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmLogEpsilonElement<TDim, TNumNodes>::load(Serializer& rSerializer)
{
...
inline std::istream& operator>>(std::istream& rIStream,
                                EvmLogEpsilonElement<TDim, TNumNodes>& rThis);

/// output stream function

template <unsigned int TDim, unsigned int TNumNodes>
inline std::ostream& operator<<(std::ostream& rOStream,
                                const EvmLogEpsilonElement<TDim, TNumNodes>& rThis)
{
...
template class EvmLogEpsilonElement<2, 3>;
template class EvmLogEpsilonElement<3, 4>;
template class EvmLogEpsilonElement<2, 4>;
template class EvmLogEpsilonElement<3, 8>;
