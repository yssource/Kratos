...
// Include Base h
#include "evm_sqrt_epsilon_element.h"
...
EvmSqrtEpsilonElement<TDim, TNumNodes>::EvmSqrtEpsilonElement(IndexType NewId)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtEpsilonElement<TDim, TNumNodes>::EvmSqrtEpsilonElement(
    EvmSqrtEpsilonElement<TDim, TNumNodes> const& rOther)
...
/// Assignment operator.
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtEpsilonElement<TDim, TNumNodes>& EvmSqrtEpsilonElement<TDim, TNumNodes>::operator=(
    EvmSqrtEpsilonElement<TDim, TNumNodes> const& rOther)
{
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmSqrtEpsilonElement<TDim, TNumNodes>::Create(
    IndexType NewId, NodesArrayType const& ThisNodes, PropertiesType::Pointer pProperties) const
{
    KRATOS_TRY
    return Kratos::make_shared<EvmSqrtEpsilonElement>(
...
{
    KRATOS_TRY
    return Kratos::make_shared<EvmSqrtEpsilonElement>(NewId, pGeom, pProperties);
    KRATOS_CATCH("");
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmSqrtEpsilonElement<TDim, TNumNodes>::Clone(IndexType NewId,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::EquationIdVector(EquationIdVectorType& rResult,
                                                              ProcessInfo& CurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rResult[i] = Element::GetGeometry()[i]
                         .GetDof(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE)
                         .EquationId();
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::GetDofList(DofsVectorType& rElementalDofList,
                                                        ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rElementalDofList[i] =
            Element::GetGeometry()[i].pGetDof(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::GetValuesVector(VectorType& rValues, int Step)
{
    this->GetFirstDerivativesVector(rValues, Step);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::GetFirstDerivativesVector(VectorType& rValues, int Step)
{
    ...
    for (IndexType iNode = 0; iNode < TNumNodes; ++iNode)
    {
        rValues[LocalIndex++] = rGeom[iNode].FastGetSolutionStepValue(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, Step);
    }
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::GetSecondDerivativesVector(VectorType& rValues,
                                                                        int Step)
{
    if (rValues.size() != TNumNodes)
        rValues.resize(TNumNodes, false);
    ...
    {
        rValues[LocalIndex++] = rGeom[iNode].FastGetSolutionStepValue(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2, Step);
    }
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix,
                                                                   ProcessInfo& rCurrentProcessInfo)
{
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateRightHandSide(VectorType& rRightHandSideVector,
                                                                    ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        const double sqrt_epsilon = this->EvaluateInPoint(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, gauss_shape_functions);
        const array_1d<double, 3> effective_convective_velocity = this->GetConvectiveVelocity(
            epsilon_sigma, sqrt_epsilon, gauss_shape_functions, r_shape_derivatives);
        ...
        const double reaction =
            this->CalculateReactionTerm(nu, y_plus, wall_distance, f2, c2, gamma, c1, c_mu, f_mu, r_shape_derivatives);
        ...
        EvmKepsilonModelUtilities::CalculateStabilizationTau(
            tau, element_length, effective_convective_velocity, contravariant_metric_tensor,
            reaction, effective_kinematic_viscosity, delta_time);

        const double source = this->CalculateSourceTerm(
            nu_t, tke, sqrt_epsilon, c1, gamma, r_shape_derivatives);

        const double s = std::sqrt(reaction);
        ...
        for (unsigned int a = 0; a < TNumNodes; ++a)
        {
            ...
            value += (velocity_convective_terms[a] + s) * tau * (source);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesContributions(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesLHS(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateFirstDerivativesRHS(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesContributions(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesLHS(
    MatrixType& rLeftHandSideMatrix, ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateSecondDerivativesRHS(
    VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    if (rRightHandSideVector.size() != 0)
        rRightHandSideVector.resize(0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateMassMatrix(MatrixType& rMassMatrix,
                                                                 ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateDampingMatrix(MatrixType& rDampingMatrix,
                                                                    ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        const array_1d<double, 3> effective_convective_velocity = this->GetConvectiveVelocity(
            epsilon_sigma, sqrt_epsilon, gauss_shape_functions, r_shape_derivatives);
        const double effective_velocity_magnitude = norm_2(effective_convective_velocity);
        ...
        array_1d<double, 3> sqrt_epsilon_gradient;
        this->CalculateGradient(sqrt_epsilon_gradient, TURBULENT_SQRT_ENERGY_DISSIPATION_RATE,
                                r_shape_derivatives);
        const double sqrt_epsilon_gradient_norm = norm_2(sqrt_epsilon_gradient);

        double cross_wind_diffusion{0.0}, stream_line_diffusion{0.0};
        const double effective_velocity_magnitude_square =
            std::pow(effective_velocity_magnitude, 2);

        if (sqrt_epsilon_gradient_norm > std::numeric_limits<double>::epsilon() &&
            effective_velocity_magnitude_square > std::numeric_limits<double>::epsilon())
        {
            ...
            const double source = this->CalculateSourceTerm(
                nu_t, tke, sqrt_epsilon, c1, gamma, r_shape_derivatives);
            Vector nodal_epsilon_sqrt;
            this->GetValuesVector(nodal_epsilon_sqrt);

            double residual = relaxed_epsilon_acceleration;
            residual += inner_prod(velocity_convective_terms, nodal_epsilon_sqrt);
            residual += reaction * inner_prod(gauss_shape_functions, nodal_epsilon_sqrt);
            residual -= source;
            residual = std::abs(residual);
            residual /= sqrt_epsilon_gradient_norm;

            double chi, k1, k2;
            EvmKepsilonModelUtilities::CalculateCrossWindDiffusionParameters(
                chi, k1, k2, effective_velocity_magnitude, tau,
                effective_kinematic_viscosity, reaction, element_length);

            stream_line_diffusion = residual * chi * k1 / effective_velocity_magnitude;
            cross_wind_diffusion = residual * chi * k2 / effective_velocity_magnitude;
        }
        ...
        for (unsigned int a = 0; a < TNumNodes; a++)
        {
            for (unsigned int b = 0; b < TNumNodes; b++)
            {
                ...
                value += 2.0 * effective_kinematic_viscosity * dNa_dNb;
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateLocalVelocityContribution(
    MatrixType& rDampingMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int iNode = 0; iNode < TNumNodes; ++iNode)
        U[iNode] = this->GetGeometry()[iNode].FastGetSolutionStepValue(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
int EvmSqrtEpsilonElement<TDim, TNumNodes>::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY

    KRATOS_ERROR_IF(this->Id() < 1)
        << "EvmSqrtEpsilonElement found with Id 0 or negative" << std::endl;

    KRATOS_ERROR_IF(this->Element::GetGeometry().Area() <= 0)
        << "On EvmSqrtEpsilonElement -> " << this->Id()
...
template <unsigned int TDim, unsigned int TNumNodes>
std::string EvmSqrtEpsilonElement<TDim, TNumNodes>::Info() const
{
    std::stringstream buffer;
    buffer << "EvmSqrtEpsilonElement #" << Element::Id();
    return buffer.str();
}
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "EvmSqrtEpsilonElement #" << Element::Id();
}

/// Print object's data.

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::PrintData(std::ostream& rOStream) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
array_1d<double, 3> EvmSqrtEpsilonElement<TDim, TNumNodes>::GetConvectiveVelocity(
    const double turbulent_energy_generation_rate_sigma,
    ...
    const Matrix& rGaussShapeDerivatives) const
{
    ...
    const double effective_kinematic_viscosity =
        nu + nu_t / turbulent_energy_generation_rate_sigma;

    double divisor = variable_value;
    if (std::abs(variable_value) <= std::numeric_limits<double>::epsilon())
        divisor = std::numeric_limits<double>::epsilon();

    return velocity - (effective_kinematic_viscosity / divisor) * sqrt_epsilon_gradient;

    return velocity;
}

template <unsigned int TDim, unsigned int TNumNodes>
double EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateReactionTerm(
...
{
    const double original_reaction_term =
        0.5 * (c2 * f2 * gamma + 2.0 * kinematic_viscosity * std::exp(-0.5 * y_plus) /
    ...
    const double source = EvmKepsilonModelUtilities::CalculateSourceTerm<TDim>(
        velocity_gradient_matrix, 1.0, 0.0);

    const double divisor = std::max(gamma, std::numeric_limits<double>::epsilon());

    const double reaction_source = 0.5 * (c1 * c_mu * f_mu * source) / divisor;

    return original_reaction_term - reaction_source;

}

template <unsigned int TDim, unsigned int TNumNodes>
double EvmSqrtEpsilonElement<TDim, TNumNodes>::CalculateSourceTerm(
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::save(Serializer& rSerializer) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtEpsilonElement<TDim, TNumNodes>::load(Serializer& rSerializer)
{
...
inline std::istream& operator>>(std::istream& rIStream,
                                EvmSqrtEpsilonElement<TDim, TNumNodes>& rThis);

/// output stream function

template <unsigned int TDim, unsigned int TNumNodes>
inline std::ostream& operator<<(std::ostream& rOStream,
                                const EvmSqrtEpsilonElement<TDim, TNumNodes>& rThis)
{
...
template class EvmSqrtEpsilonElement<2, 3>;
template class EvmSqrtEpsilonElement<3, 4>;
template class EvmSqrtEpsilonElement<2, 4>;
template class EvmSqrtEpsilonElement<3, 8>;
