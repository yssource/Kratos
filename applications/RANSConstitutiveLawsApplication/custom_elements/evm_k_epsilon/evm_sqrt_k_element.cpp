...
EvmSqrtKElement<TDim, TNumNodes>::EvmSqrtKElement(IndexType NewId)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtKElement<TDim, TNumNodes>::EvmSqrtKElement(IndexType NewId, const NodesArrayType& ThisNodes)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtKElement<TDim, TNumNodes>::EvmSqrtKElement(IndexType NewId, GeometryType::Pointer pGeometry)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtKElement<TDim, TNumNodes>::EvmSqrtKElement(EvmSqrtKElement<TDim, TNumNodes> const& rOther)
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtKElement<TDim, TNumNodes>::~EvmSqrtKElement()
{
}
...
/// Assignment operator.
template <unsigned int TDim, unsigned int TNumNodes>
EvmSqrtKElement<TDim, TNumNodes>& EvmSqrtKElement<TDim, TNumNodes>::operator=(
    EvmSqrtKElement<TDim, TNumNodes> const& rOther)
{
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmSqrtKElement<TDim, TNumNodes>::Create(IndexType NewId,
                                                          NodesArrayType const& ThisNodes,
                                                          PropertiesType::Pointer pProperties) const
{
    KRATOS_TRY
    return Kratos::make_shared<EvmSqrtKElement>(
...
{
    KRATOS_TRY
    return Kratos::make_shared<EvmSqrtKElement>(NewId, pGeom, pProperties);
    KRATOS_CATCH("");
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
Element::Pointer EvmSqrtKElement<TDim, TNumNodes>::Clone(IndexType NewId,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::EquationIdVector(EquationIdVectorType& rResult,
                                                        ProcessInfo& CurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rResult[i] =
            Element::GetGeometry()[i].GetDof(TURBULENT_SQRT_KINETIC_ENERGY).EquationId();
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::GetDofList(DofsVectorType& rElementalDofList,
                                                  ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int i = 0; i < TNumNodes; i++)
        rElementalDofList[i] =
            Element::GetGeometry()[i].pGetDof(TURBULENT_SQRT_KINETIC_ENERGY);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::GetValuesVector(VectorType& rValues, int Step)
{
    this->GetFirstDerivativesVector(rValues, Step);
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::GetFirstDerivativesVector(VectorType& rValues, int Step)
{
    ...
    {
        rValues[LocalIndex++] =
            rGeom[iNode].FastGetSolutionStepValue(TURBULENT_SQRT_KINETIC_ENERGY, Step);
    }
}

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::GetSecondDerivativesVector(VectorType& rValues, int Step)
{
    if (rValues.size() != TNumNodes)
        rValues.resize(TNumNodes, false);
    ...
    for (IndexType iNode = 0; iNode < TNumNodes; ++iNode)
    {
        rValues[LocalIndex++] = rGeom[iNode].FastGetSolutionStepValue(
            TURBULENT_SQRT_KINETIC_ENERGY_RATE, Step);
    }
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateLocalSystem(MatrixType& rLeftHandSideMatrix,
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateLeftHandSide(MatrixType& rLeftHandSideMatrix,
                                                             ProcessInfo& rCurrentProcessInfo)
{
}
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateRightHandSide(VectorType& rRightHandSideVector,
                                                              ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        const double sqrt_tke = this->EvaluateInPoint(
            TURBULENT_SQRT_KINETIC_ENERGY, gauss_shape_functions);
        const array_1d<double, 3> effective_convective_velocity = this->GetConvectiveVelocity(
            tke_sigma, sqrt_tke, gauss_shape_functions, r_shape_derivatives);
        ...
        const double source =
            this->CalculateSourceTerm(nu_t, tke, sqrt_tke, r_shape_derivatives);
        ...
        const double reaction = this->CalculateReactionTerm(nu, wall_distance, gamma, c_mu, f_mu, r_shape_derivatives);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateFirstDerivativesContributions(
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateFirstDerivativesLHS(MatrixType& rLeftHandSideMatrix,
                                                                    ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateFirstDerivativesRHS(VectorType& rRightHandSideVector,
                                                                    ProcessInfo& rCurrentProcessInfo)
{
    if (rRightHandSideVector.size() != 0)
        rRightHandSideVector.resize(0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateSecondDerivativesContributions(
    MatrixType& rLeftHandSideMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateSecondDerivativesLHS(MatrixType& rLeftHandSideMatrix,
                                                                     ProcessInfo& rCurrentProcessInfo)
{
    if (rLeftHandSideMatrix.size1() != 0)
        rLeftHandSideMatrix.resize(0, 0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateSecondDerivativesRHS(VectorType& rRightHandSideVector,
                                                                     ProcessInfo& rCurrentProcessInfo)
{
    if (rRightHandSideVector.size() != 0)
        rRightHandSideVector.resize(0, false);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateMassMatrix(MatrixType& rMassMatrix,
                                                           ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateDampingMatrix(MatrixType& rDampingMatrix,
                                                              ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY
    ...
    for (unsigned int g = 0; g < num_gauss_points; g++)
    {
        ...
        array_1d<double, 3> sqrt_tke_gradient;
        this->CalculateGradient(sqrt_tke_gradient, TURBULENT_SQRT_KINETIC_ENERGY,
                                r_shape_derivatives);
        const double sqrt_tke_gradient_norm = norm_2(sqrt_tke_gradient);
        ...
        if (sqrt_tke_gradient_norm > std::numeric_limits<double>::epsilon() &&
            effective_velocity_magnitude_square > std::numeric_limits<double>::epsilon())
        {
            ...
            const double source =
                this->CalculateSourceTerm(nu_t, tke, sqrt_tke, r_shape_derivatives);
            Vector nodal_tke_sqrt;
            this->GetValuesVector(nodal_tke_sqrt);

            double residual = relaxed_tke_acceleration;
            residual += inner_prod(velocity_convective_terms, nodal_tke_sqrt);
            residual += reaction * inner_prod(gauss_shape_functions, nodal_tke_sqrt);
            residual -= source;
            residual = std::abs(residual);
            residual /= sqrt_tke_gradient_norm;
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::CalculateLocalVelocityContribution(
    MatrixType& rDampingMatrix, VectorType& rRightHandSideVector, ProcessInfo& rCurrentProcessInfo)
{
    ...
    for (unsigned int iNode = 0; iNode < TNumNodes; ++iNode)
        U[iNode] = this->GetGeometry()[iNode].FastGetSolutionStepValue(TURBULENT_SQRT_KINETIC_ENERGY);
 ...
 */
template <unsigned int TDim, unsigned int TNumNodes>
int EvmSqrtKElement<TDim, TNumNodes>::Check(const ProcessInfo& rCurrentProcessInfo)
{
    KRATOS_TRY

    KRATOS_ERROR_IF(this->Id() < 1)
        << "EvmSqrtKElement found with Id 0 or negative" << std::endl;

    KRATOS_ERROR_IF(this->Element::GetGeometry().Area() <= 0)
        << "On EvmSqrtKElement -> " << this->Id()
...
template <unsigned int TDim, unsigned int TNumNodes>
std::string EvmSqrtKElement<TDim, TNumNodes>::Info() const
{
    std::stringstream buffer;
    buffer << "EvmSqrtKElement #" << Element::Id();
    return buffer.str();
}
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::PrintInfo(std::ostream& rOStream) const
{
    rOStream << "EvmSqrtKElement #" << Element::Id();
}

/// Print object's data.

template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::PrintData(std::ostream& rOStream) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
array_1d<double, 3> EvmSqrtKElement<TDim, TNumNodes>::GetConvectiveVelocity(
    ...
    const Matrix& rGaussShapeDerivatives) const
{
    ...
    return velocity - (effective_kinematic_viscosity / divisor) * sqrt_tke_gradient;
}

template <unsigned int TDim, unsigned int TNumNodes>
double EvmSqrtKElement<TDim, TNumNodes>::CalculateReactionTerm(const double kinematic_viscosity,
                                                               ...
                                                               const Matrix& rShapeDerivatives) const
{
    const double original_reaction_term =
        0.5 * (2.0 * kinematic_viscosity / std::pow(wall_distance, 2) + gamma);
    ...
    const double source = EvmKepsilonModelUtilities::CalculateSourceTerm<TDim>(velocity_gradient_matrix, 1.0, 0.0);

    const double divisor = std::max(gamma, std::numeric_limits<double>::epsilon());

    const double reaction_source = 0.5 * c_mu * f_mu * source / divisor;

    return original_reaction_term - reaction_source;
}

template <unsigned int TDim, unsigned int TNumNodes>
double EvmSqrtKElement<TDim, TNumNodes>::CalculateSourceTerm(const double turbulent_kinematic_viscosity,
                                                             ...
                                                             const Matrix& rShapeDerivatives) const
{
    ...
    // return (tke_production / (2.0 * divisor));
    return 0.0;
}
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::save(Serializer& rSerializer) const
{
...
template <unsigned int TDim, unsigned int TNumNodes>
void EvmSqrtKElement<TDim, TNumNodes>::load(Serializer& rSerializer)
{
...
inline std::istream& operator>>(std::istream& rIStream,
                                EvmSqrtKElement<TDim, TNumNodes>& rThis);

/// output stream function

template <unsigned int TDim, unsigned int TNumNodes>
inline std::ostream& operator<<(std::ostream& rOStream,
                                const EvmSqrtKElement<TDim, TNumNodes>& rThis)
{
...
template class EvmSqrtKElement<2, 3>;
template class EvmSqrtKElement<3, 4>;
template class EvmSqrtKElement<2, 4>;
template class EvmSqrtKElement<3, 8>;
