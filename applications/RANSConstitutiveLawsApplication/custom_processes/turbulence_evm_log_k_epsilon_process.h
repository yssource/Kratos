...
#if !defined(KRATOS_TURBULENCE_EVM_LOG_K_EPSILON_PROCESS_H_INCLUDED)
#define KRATOS_TURBULENCE_EVM_LOG_K_EPSILON_PROCESS_H_INCLUDED
...
#include "custom_strategies/evm_k_epsilon/residual_based_bossak_log_turbulent_energy_dissipation_scheme.h"
#include "custom_strategies/evm_k_epsilon/residual_based_bossak_log_turbulent_kinetic_energy_scheme.h"
...
class TurbulenceEvmLogKEpsilonProcess
...
public:
    ...
    /// Pointer definition of TurbulenceEvmLogKEpsilonProcess
    KRATOS_CLASS_POINTER_DEFINITION(TurbulenceEvmLogKEpsilonProcess);
    ...
    /// Constructor.
    TurbulenceEvmLogKEpsilonProcess(ModelPart& rModelPart,
          ...
          mpEpsilonLinearSolver(pEpsilonLinearSolver)
    {
        ...
        KRATOS_INFO("TurbulenceModel")
            << "Initializing log k-epsilon turbulence model" << std::endl;
    ...
    /// Destructor.
    virtual ~TurbulenceEvmLogKEpsilonProcess()
    {
    }
    ...
    {
        return std::string("TurbulenceEvmLogKEpsilonProcess");
    }
...
protected:
    ...
    void InitializeTurbulenceModelPart() override
    {
        ...
        const Element& k_element =
            KratosComponents<Element>::Get("RANSEVMLOGK" + element_postfix.str());
        const Element& epsilon_element =
            KratosComponents<Element>::Get("RANSEVMLOGEPSILON" + element_postfix.str());
    ...
    void AddSolutionStepVariables() override
    {
        ...
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(TURBULENT_LOG_KINETIC_ENERGY);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(TURBULENT_LOG_KINETIC_ENERGY_RATE);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2);
    ...
    {
        VariableUtils().AddDof<Variable<double>>(TURBULENT_LOG_KINETIC_ENERGY, this->mrModelPart);
        VariableUtils().AddDof<Variable<double>>(
            TURBULENT_LOG_ENERGY_DISSIPATION_RATE, this->mrModelPart);
        BaseType::AddDofs();
    }
    ///@}
...
private:
    ...
    void GenerateSolutionStrategies()
    {
        ...
        mpKScheme = SchemePointerType(
            new ResidualBasedBossakLogTurbulentKineticEnergyScheme<TSparseSpace, TDenseSpace>(
                alpha_bossak));
        ...
        mpEpsilonScheme = SchemePointerType(
            new ResidualBasedBossakLogTurbulentEnergyDissipationRateScheme<TSparseSpace, TDenseSpace>(
                alpha_bossak));
...
#pragma omp parallel for reduction(+ : initialized_nodes_count)
        for (int i = 0; i < number_of_nodes; i++)
        {
            ...
            if (!i_node->IsFixed(TURBULENT_LOG_KINETIC_ENERGY) &&
                !i_node->IsFixed(TURBULENT_LOG_ENERGY_DISSIPATION_RATE))
            {
...
#pragma omp parallel for reduction(+ : nodes_count_fixed, nodes_count_free)
        for (int i = 0; i < number_of_nodes; i++)
        {
            ...
            {
                i_node->Fix(TURBULENT_LOG_KINETIC_ENERGY);
                i_node->Fix(TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
                nodes_count_fixed++;
            }
            else
            {
                i_node->Free(TURBULENT_LOG_KINETIC_ENERGY);
                i_node->Free(TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
                nodes_count_free++;
            }
        }
    ...
    void InitializeValues(NodeType& rNode, double K, double Epsilon)
    {
        ...
        rNode.FastGetSolutionStepValue(TURBULENT_LOG_KINETIC_ENERGY) = std::log(K);
        rNode.FastGetSolutionStepValue(TURBULENT_LOG_ENERGY_DISSIPATION_RATE) = std::log(Epsilon);
    }
...
#pragma omp parallel for
    ...
    void SolveStep()
    {
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_LOG_KINETIC_ENERGY, "SolveStep");
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_LOG_ENERGY_DISSIPATION_RATE, "SolveStep");
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_KINETIC_ENERGY, "SolveStep");
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_ENERGY_DISSIPATION_RATE, "SolveStep");
...
#pragma omp parallel for reduction(+ : increase_norm, solution_norm)
    ...
    {
        const double log_tke = iNode->FastGetSolutionStepValue(TURBULENT_LOG_KINETIC_ENERGY, Step);
        const double log_epsilon =
            iNode->FastGetSolutionStepValue(TURBULENT_LOG_ENERGY_DISSIPATION_RATE, Step);
        const double y_plus = iNode->FastGetSolutionStepValue(RANS_Y_PLUS);
        const double f_mu = EvmLogKepsilonModelUtilities::CalculateFmu(y_plus);

        const double nu_t = EvmLogKepsilonModelUtilities::CalculateTurbulentViscosity(C_mu, log_tke, log_epsilon, f_mu);
...
#pragma omp parallel for
        ...
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_VISCOSITY, "NuT");
    }
    ...
    /// Assignment operator.
    TurbulenceEvmLogKEpsilonProcess& operator=(TurbulenceEvmLogKEpsilonProcess const& rOther);

    /// Copy constructor.
    TurbulenceEvmLogKEpsilonProcess(TurbulenceEvmLogKEpsilonProcess const& rOther);
...
inline std::istream& operator>>(
    ...
    TurbulenceEvmLogKEpsilonProcess<TDim, TSparseSpace, TDenseSpace, TLinearSolver>& rThis);

/// output stream function
template <unsigned int TDim, class TSparseSpace, class TDenseSpace, class TLinearSolver>
inline std::ostream& operator<<(
    std::ostream& rOStream,
    const TurbulenceEvmLogKEpsilonProcess<TDim, TSparseSpace, TDenseSpace, TLinearSolver>& rThis)
{
...
#endif // KRATOS_TURBULENCE_EVM_LOG_K_EPSILON_PROCESS_H_INCLUDED  defined
