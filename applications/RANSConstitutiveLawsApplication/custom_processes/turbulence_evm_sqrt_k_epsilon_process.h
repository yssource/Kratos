...
#if !defined(KRATOS_TURBULENCE_EVM_SQRT_K_EPSILON_PROCESS_H_INCLUDED)
#define KRATOS_TURBULENCE_EVM_SQRT_K_EPSILON_PROCESS_H_INCLUDED
...
#include "custom_strategies/evm_k_epsilon/residual_based_bossak_sqrt_turbulent_energy_dissipation_scheme.h"
#include "custom_strategies/evm_k_epsilon/residual_based_bossak_sqrt_turbulent_kinetic_energy_scheme.h"
...
class TurbulenceEvmSqrtKEpsilonProcess
...
public:
    ...
    /// Pointer definition of TurbulenceEvmSqrtKEpsilonProcess
    KRATOS_CLASS_POINTER_DEFINITION(TurbulenceEvmSqrtKEpsilonProcess);
    ...
    /// Constructor.
    TurbulenceEvmSqrtKEpsilonProcess(ModelPart& rModelPart,
          ...
          mpEpsilonLinearSolver(pEpsilonLinearSolver)
    {
        ...
        KRATOS_INFO("TurbulenceModel")
            << "Initializing sqrt k-epsilon turbulence model" << std::endl;
    ...
    /// Destructor.
    virtual ~TurbulenceEvmSqrtKEpsilonProcess()
    {
    }
...
#pragma omp parallel for
    ...
    {
        return std::string("TurbulenceEvmSqrtKEpsilonProcess");
    }
...
protected:
    ...
    void InitializeTurbulenceModelPart() override
    {
        ...
        const Element& k_element =
            KratosComponents<Element>::Get("RANSEVMSQRTK" + element_postfix.str());
        const Element& epsilon_element =
            KratosComponents<Element>::Get("RANSEVMSQRTEPSILON" + element_postfix.str());
    ...
    void AddSolutionStepVariables() override
    {
        ...
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(TURBULENT_SQRT_KINETIC_ENERGY);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(TURBULENT_SQRT_KINETIC_ENERGY_RATE);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
        this->mrModelPart.GetNodalSolutionStepVariablesList().push_back(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2);
    ...
    {
        VariableUtils().AddDof<Variable<double>>(TURBULENT_SQRT_KINETIC_ENERGY, this->mrModelPart);
        VariableUtils().AddDof<Variable<double>>(
            TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, this->mrModelPart);
        BaseType::AddDofs();
    }
    ///@}
...
private:
    ...
    void GenerateSolutionStrategies()
    {
        ...
        mpKScheme = SchemePointerType(
            new ResidualBasedBossakSqrtTurbulentKineticEnergyScheme<TSparseSpace, TDenseSpace>(
                alpha_bossak));
        ...
        mpEpsilonScheme = SchemePointerType(
            new ResidualBasedBossakSqrtTurbulentEnergyDissipationRateScheme<TSparseSpace, TDenseSpace>(
                alpha_bossak));
...
#pragma omp parallel for reduction(+ : initialized_nodes_count)
        for (int i = 0; i < number_of_nodes; i++)
        {
            ...
            if (!i_node->IsFixed(TURBULENT_SQRT_KINETIC_ENERGY) &&
                !i_node->IsFixed(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE))
            {
...
#pragma omp parallel for reduction(+ : nodes_count_fixed, nodes_count_free)
        for (int i = 0; i < number_of_nodes; i++)
        {
            ...
            {
                i_node->Fix(TURBULENT_SQRT_KINETIC_ENERGY);
                i_node->Fix(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
                nodes_count_fixed++;
            }
            else
            {
                i_node->Free(TURBULENT_SQRT_KINETIC_ENERGY);
                i_node->Free(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
                nodes_count_free++;
            }
        }
    ...
    void InitializeValues(NodeType& rNode, double K, double Epsilon)
    {
        ...
        rNode.FastGetSolutionStepValue(TURBULENT_SQRT_KINETIC_ENERGY) = std::sqrt(K);
        rNode.FastGetSolutionStepValue(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE) = std::sqrt(Epsilon);
    }
...
#pragma omp parallel for
        for (int i_cond = 0; i_cond < number_of_conditions; ++i_cond)
        {
            ...
            for (unsigned int i_node = 0; i_node < number_of_condition_nodes; i_node++)
            {
                ...
                const double tke = std::pow(u_tau, 2) / std::sqrt(c_mu);
                const double epsilon = std::pow(u_tau, 4) / (von_karman * y_plus * nu);
                this->InitializeValues(r_condition_node, tke, epsilon);
    ...
    void SolveStep()
    {
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_SQRT_KINETIC_ENERGY, "SolveStep");
        CalculationUtilities::CheckBounds<NodeType>(this->mrModelPart, TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, "SolveStep");
...
#pragma omp parallel for
    ...
    /// Assignment operator.
    TurbulenceEvmSqrtKEpsilonProcess& operator=(TurbulenceEvmSqrtKEpsilonProcess const& rOther);

    /// Copy constructor.
    TurbulenceEvmSqrtKEpsilonProcess(TurbulenceEvmSqrtKEpsilonProcess const& rOther);
...
inline std::istream& operator>>(
    ...
    TurbulenceEvmSqrtKEpsilonProcess<TDim, TSparseSpace, TDenseSpace, TLinearSolver>& rThis);

/// output stream function
template <unsigned int TDim, class TSparseSpace, class TDenseSpace, class TLinearSolver>
inline std::ostream& operator<<(
    std::ostream& rOStream,
    const TurbulenceEvmSqrtKEpsilonProcess<TDim, TSparseSpace, TDenseSpace, TLinearSolver>& rThis)
{
...
#endif // KRATOS_TURBULENCE_EVM_SQRT_K_EPSILON_PROCESS_H_INCLUDED  defined
