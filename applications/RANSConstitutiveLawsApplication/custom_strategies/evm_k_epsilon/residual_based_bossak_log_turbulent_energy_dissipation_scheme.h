...
#if !defined(KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED)
#define KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED
...
class ResidualBasedBossakLogTurbulentEnergyDissipationRateScheme
    : public ResidualBasedBossakVelocityScheme<TSparseSpace, TDenseSpace>
{
    ...
    public:
                                       ...
                                       ProcessInfo& rCurrentProcessInfo) override
        {
            ...
            rVector[0] =
                MakeIndirectScalar(r_node, TURBULENT_LOG_ENERGY_DISSIPATION_RATE, Step);
        }
                                        ...
                                        ProcessInfo& rCurrentProcessInfo) override
        {
            ...
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] =
                MakeIndirectScalar(r_node, TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2, Step);
        }
                                           ...
                                           ProcessInfo& rCurrentProcessInfo) override
        {
            rVector.resize(1);
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] = r_node.pGetDof(TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_LOG_ENERGY_DISSIPATION_RATE;
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2;
        }
    };
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(ResidualBasedBossakLogTurbulentEnergyDissipationRateScheme);
    ...
    ResidualBasedBossakLogTurbulentEnergyDissipationRateScheme(const double AlphaBossak)
...
#pragma omp parallel for
        ...
        KRATOS_INFO("LogEpsilonScheme") << "Initialized.\n";
...
#pragma omp parallel for
        for (int iNode = 0; iNode < number_of_nodes; ++iNode)
        {
            ...
            const double epsilon_dot_old = r_node.FastGetSolutionStepValue(
                TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2, 1);
            const double epsilon_dot = r_node.FastGetSolutionStepValue(
                TURBULENT_LOG_ENERGY_DISSIPATION_RATE_2, 0);
            ...
            const double log_epsilon = r_node.FastGetSolutionStepValue(TURBULENT_LOG_ENERGY_DISSIPATION_RATE);
            r_node.FastGetSolutionStepValue(TURBULENT_ENERGY_DISSIPATION_RATE) = std::exp(log_epsilon);
        }

        CalculationUtilities::CheckBounds<NodeType>(rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE);
...
#endif // KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED defined