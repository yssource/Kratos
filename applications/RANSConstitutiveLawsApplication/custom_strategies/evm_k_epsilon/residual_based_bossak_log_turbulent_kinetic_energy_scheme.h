...
#if !defined(KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_KINETIC_ENERGY_SCHEME_H_INCLUDED)
#define KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_KINETIC_ENERGY_SCHEME_H_INCLUDED
...
class ResidualBasedBossakLogTurbulentKineticEnergyScheme
    : public ResidualBasedBossakVelocityScheme<TSparseSpace, TDenseSpace>
{
    ...
    public:
                                       ...
                                       ProcessInfo& rCurrentProcessInfo) override
        {
            ...
            rVector[0] = MakeIndirectScalar(r_node, TURBULENT_LOG_KINETIC_ENERGY, Step);
        }
                                        ...
                                        ProcessInfo& rCurrentProcessInfo) override
        {
            rVector.resize(1);
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] = MakeIndirectScalar(r_node, TURBULENT_LOG_KINETIC_ENERGY_RATE, Step);
        }
                                           ...
                                           ProcessInfo& rCurrentProcessInfo) override
        {
            rVector.resize(1);
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] = r_node.pGetDof(TURBULENT_LOG_KINETIC_ENERGY);
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_LOG_KINETIC_ENERGY;
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_LOG_KINETIC_ENERGY_RATE;
        }
    };
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(ResidualBasedBossakLogTurbulentKineticEnergyScheme);
    ...
    ResidualBasedBossakLogTurbulentKineticEnergyScheme(const double AlphaBossak)
...
#pragma omp parallel for
        ...
        KRATOS_INFO("LogKScheme") << "Initialized.\n";
...
#pragma omp parallel for
        for (int iNode = 0; iNode < number_of_nodes; ++iNode)
        {
            ...
            const double tke_dot_old =
                r_node.FastGetSolutionStepValue(TURBULENT_LOG_KINETIC_ENERGY_RATE, 1);
            const double tke_dot =
                r_node.FastGetSolutionStepValue(TURBULENT_LOG_KINETIC_ENERGY_RATE, 0);

            r_node.FastGetSolutionStepValue(RANS_AUXILIARY_VARIABLE_1) =
                this->mAlphaBossak * tke_dot_old + (1.0 - this->mAlphaBossak) * tke_dot;

            const double log_k = r_node.FastGetSolutionStepValue(TURBULENT_LOG_KINETIC_ENERGY);
            r_node.FastGetSolutionStepValue(TURBULENT_KINETIC_ENERGY) = std::exp(log_k);
        }

        CalculationUtilities::CheckBounds<NodeType>(rModelPart, TURBULENT_KINETIC_ENERGY);
...
#endif // KRATOS_RESIDUAL_BASED_BOSSAK_LOG_TURBULENT_KINETIC_ENERGY_SCHEME_H_INCLUDED defined