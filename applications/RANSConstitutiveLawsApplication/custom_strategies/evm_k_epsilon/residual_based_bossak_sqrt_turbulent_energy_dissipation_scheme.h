...
#if !defined(KRATOS_RESIDUAL_BASED_BOSSAK_SQRT_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED)
#define KRATOS_RESIDUAL_BASED_BOSSAK_SQRT_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED
...
class ResidualBasedBossakSqrtTurbulentEnergyDissipationRateScheme
    : public ResidualBasedBossakVelocityScheme<TSparseSpace, TDenseSpace>
{
    ...
    public:
                                       ...
                                       ProcessInfo& rCurrentProcessInfo) override
        {
            ...
            rVector[0] =
                MakeIndirectScalar(r_node, TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, Step);
        }
                                        ...
                                        ProcessInfo& rCurrentProcessInfo) override
        {
            ...
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] =
                MakeIndirectScalar(r_node, TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2, Step);
        }
                                           ...
                                           ProcessInfo& rCurrentProcessInfo) override
        {
            rVector.resize(1);
            NodeType& r_node = mpElement->GetGeometry()[NodeId];
            rVector[0] = r_node.pGetDof(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_SQRT_ENERGY_DISSIPATION_RATE;
        }
        ...
        {
            rVariables.resize(1);
            rVariables[0] = &TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2;
        }
    };
...
public:
    ...
    KRATOS_CLASS_POINTER_DEFINITION(ResidualBasedBossakSqrtTurbulentEnergyDissipationRateScheme);
    ...
    ResidualBasedBossakSqrtTurbulentEnergyDissipationRateScheme(const double AlphaBossak)
...
#pragma omp parallel for
        ...
        KRATOS_INFO("SqrtEpsilonScheme") << "Initialized.\n";
...
//             double& sqrt_epsilon = r_node.FastGetSolutionStepValue(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
//             sqrt_epsilon = std::abs(sqrt_epsilon);
//         }
...
#pragma omp parallel for
        for (int iNode = 0; iNode < number_of_nodes; ++iNode)
        {
            ...
            const double epsilon_dot_old = r_node.FastGetSolutionStepValue(
                TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2, 1);
            const double epsilon_dot = r_node.FastGetSolutionStepValue(
                TURBULENT_SQRT_ENERGY_DISSIPATION_RATE_2, 0);
            ...
            const double sqrt_epsilon = r_node.FastGetSolutionStepValue(TURBULENT_SQRT_ENERGY_DISSIPATION_RATE);
            r_node.FastGetSolutionStepValue(TURBULENT_ENERGY_DISSIPATION_RATE) = std::pow(sqrt_epsilon, 2);
        }

        CalculationUtilities::CheckBounds<NodeType>(rModelPart, TURBULENT_SQRT_ENERGY_DISSIPATION_RATE, "Update");
        CalculationUtilities::CheckBounds<NodeType>(rModelPart, TURBULENT_ENERGY_DISSIPATION_RATE, "Update");
...
#endif // KRATOS_RESIDUAL_BASED_BOSSAK_SQRT_TURBULENT_ENERGY_DISSIPATION_RATE_SCHEME_H_INCLUDED defined