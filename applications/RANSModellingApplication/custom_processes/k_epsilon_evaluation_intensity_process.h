...
#if !defined(KRATOS_RANS_K_EPSILON_EVALUATION_INTENSITY_PROCESS_H_INCLUDED)
#define KRATOS_RANS_K_EPSILON_EVALUATION_INTENSITY_PROCESS_H_INCLUDED
...
class RansKEpsilonEvaluationIntensityProcess : public Process
{
public:
    ///@name Type Definitions
    ///@{
    ...
    /// Pointer definition of RansKEpsilonEvaluationIntensityProcess
    KRATOS_CLASS_POINTER_DEFINITION(RansKEpsilonEvaluationIntensityProcess);
    ...
    /// Constructor
    RansKEpsilonEvaluationIntensityProcess(Model& rModel, Parameters& rParameters)
        : mrModel(rModel), mrParameters(rParameters)
    {
        ...
        mTurbulentEnergyDissipationMin =
            mrParameters["variable_minimums"]
                        ["turbulent_energy_dissipation_rate"].GetDouble();
    ...
    /// Destructor.
    ~RansKEpsilonEvaluationIntensityProcess() override
    {
...
#pragma omp parallel for
    ...
    {
        return std::string("RansKEpsilonEvaluationIntensityProcess");
    }
...
#pragma omp parallel for
    ...
    {
        const double velocity_magnitude = norm_2(rNode.FastGetSolutionStepValue(VELOCITY));
        ...
        tke = 1.5 * std::pow(mTurbulentIntensity * velocity_magnitude,2 );
        epsilon = std::pow(mCmu, 0.75) * std::pow(tke, 1.5) / mTurbulentMixingLength;
    }
    ...
    /// Assignment operator.
    RansKEpsilonEvaluationIntensityProcess& operator=(RansKEpsilonEvaluationIntensityProcess const& rOther);

    /// Copy constructor.
    RansKEpsilonEvaluationIntensityProcess(RansKEpsilonEvaluationIntensityProcess const& rOther);

    ///@}

}; // Class RansKEpsilonEvaluationIntensityProcess
...
inline std::ostream& operator<<(std::ostream& rOStream,
                                const RansKEpsilonEvaluationIntensityProcess& rThis);
...
#endif // KRATOS_RANS_K_EPSILON_EVALUATION_INTENSITY_PROCESS_H_INCLUDED  defined
