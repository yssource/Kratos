...
#if !defined(KRATOS_RANS_DRAG_RESPONSE_FUNCTION_H_INCLUDED)
#define KRATOS_RANS_DRAG_RESPONSE_FUNCTION_H_INCLUDED
...
class RansDragResponseFunction : public AdjointResponseFunction
{
public:
    ///@name Type Definitions
    ///@{

    KRATOS_CLASS_POINTER_DEFINITION(RansDragResponseFunction);
    ...
    /// Constructor.
    RansDragResponseFunction(Parameters Settings, ModelPart& rModelPart)
        : mrModelPart(rModelPart)
    {
        ...
        if (std::abs(norm_2(mDragDirection) - 1.0) > 1e-3)
        {
            ...
            KRATOS_WARNING("RansDragResponseFunction")
                << "Non unit magnitude in \"drag_direction\"." << std::endl;
            KRATOS_WARNING("RansDragResponseFunction") << "Normalizing ..." << std::endl;
    ...
    /// Destructor.
    ~RansDragResponseFunction() override
    {
    }
    ...
    {
        KRATOS_TRY;
        KRATOS_ERROR << "RansDragResponseFunction::CalculateValue(ModelPart& "
...
#endif /* KRATOS_RANS_DRAG_RESPONSE_FUNCTION_H_INCLUDED defined */
