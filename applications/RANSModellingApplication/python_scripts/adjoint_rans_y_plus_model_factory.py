...
def Factory(settings, Model):
    ...
    if (not y_plus_model_name in y_plus_model_list):
        raise Exception("Unknown adjoint y_plus \"model_type\" name: \"" + y_plus_model_name + "\".\nAllowed \"model_type\" names: " + y_plus_model_list.__str__())

    if y_plus_model_name == "logarithmic":
        return KratosRANS.RansLogarithmicYPlusModelSensitivitiesProcess(model_part, settings["model_settings"])
...
def InitializeModelPartName(settings, Model, model_part):
    if (not settings.Has("model_part_name")):
        ...
        if model_part_name != model_part.Name:
            KratosMultiphysics.Logger.PrintWarning("AdjointRANSYPlusModelSettings", "Using a different model part (i.e. \"" + model_part_name + "\") over the intended model part \"" + model_part.Name + "\"")
