...
class EmpireMapper():
    ...
    def __init__(self, origin_model_part, destination_model_part, settings):
        ...
        self.name = (self.__GetMapperType()+str(EmpireMapper.count)).encode(encoding='UTF-8')

        self.origin_model_part = origin_model_part
        self.origin_model_part_name = (origin_model_part.Name+str(EmpireMapper.count)).encode(encoding='UTF-8')
        self.destination_model_part = destination_model_part
        if destination_model_part.Name == origin_model_part.Name:
            self.destination_model_part_name = (destination_model_part.Name+str(EmpireMapper.count) + "_destination").encode(encoding='UTF-8')
        else:
            self.destination_model_part_name = (destination_model_part.Name+str(EmpireMapper.count)).encode(encoding='UTF-8')
        ...
        EmpireMapper.count +=1

    def __GetMapperType(self):
        return "Kratos_Empire_Mapper_Base_"
...
class EmpireVertexMorphingMapper(EmpireMapper):
    ...
    def __GetMapperType(self):
        return "Kratos_Empire_Mapper_VertexMorphing"
...
class EmpireFEMortarMapper(EmpireMapper):
    ...
    def __GetMapperType(self):
        return "Kratos_Empire_Mapper_FEMortar"