...
#include "solving_strategies/schemes/residual_based_generalized_alpha_custom_scheme.h"
...
//                 std::cout << time << "\t" << x << "\t" << v << "\t" << a << std::endl;
        ...
        /**
         * Checks if the generalized alpha scheme performs correctly the integration
         */

        KRATOS_TEST_CASE_IN_SUITE(CustomGeneralizedAlphaSchemeTest, KratosCoreSchemesFastSuite)
        {
            constexpr double tolerance = 1e-6;

            ModelPart model_part("Main");

            typedef ResidualBasedGeneralizedAlphaCustomScheme< SparseSpaceType, LocalSpaceType >  ResidualBasedGeneralizedAlphaCustomSchemeType;
            SchemeType::Pointer pscheme = SchemeType::Pointer( new ResidualBasedGeneralizedAlphaCustomSchemeType() );
